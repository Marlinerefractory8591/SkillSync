...
export const UpdateCenterModal: React.FC = () => {
  ...
  const upToDate = skills.filter((s) => !s.updateAvailable && s.status !== 'error');
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-foreground">{t('updateCenterTitle')}</h2>
            </div>
            <button
              onClick={closeUpdateCenter}
              ...
              <div className="p-4 rounded-xl border border-primary/30 bg-primary/10 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-foreground">
                      {t('batchInProgress')} ({batchProgress.completed}/{batchProgress.total})
                    </span>
                  </div>
                ...
                {batchProgress.currentItem && (
                  <p className="text-[11px] text-muted-foreground font-mono">
                    {t('currentlyUpdating')}: <strong className="text-foreground">{batchProgress.currentItem}</strong>
                  </p>
                )}
              </div>
            )}
                    ...
                    >
                      <div>
                        ...
                        <p className="text-[11px] font-mono text-muted-foreground">
                          {formatSkillVersion(s.currentVersion, language)} ➔ <span className="text-amber-400 font-semibold">{formatSkillVersion(s.latestVersion, language)}</span>
                        </p>
                      </div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {upToDate.map((s) => (
                  ...
                  >
                    ...
                    <span className="font-mono text-muted-foreground text-[11px]">{formatSkillVersion(s.currentVersion, language)}</span>
                  </div>
                ))}
              </div>
            </div>
