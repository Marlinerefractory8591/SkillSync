    ...
    >
      <div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          ...
          {skill.updateAvailable && skill.updateCompatibility && (
            ...
            >
              {skill.updateCompatibility.includes('Major') ? '⚠️ SemVer Major' : '✓ SemVer Minor'}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="mt-5 pt-3 border-t border-border/60 flex flex-wrap items-center justify-between gap-2 text-xs">
        ...
        <div className="flex items-center gap-1 font-mono text-[11px] shrink-0">
          <span className="text-muted-foreground">{formatSkillVersion(skill.currentVersion, language)}</span>
          {skill.updateAvailable && skill.latestVersion && skill.latestVersion !== skill.currentVersion && (
            <>
              <ArrowRight className="w-3 h-3 text-amber-400" />
              <span className="text-amber-400 font-semibold">{formatSkillVersion(skill.latestVersion, language)}</span>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap ml-auto" onClick={(e) => e.stopPropagation()}>
          ...
          {skill.remoteUrl && (
            ...
            >
              <RefreshCw className={`w-3 h-3 ${isCheckingGitHub ? 'animate-spin text-primary' : ''}`} />
            </button>
          )}
          ...
          {skill.updateAvailable && (
            ...
            >
              {isUpdating ? <Loader2 className="w-3 h-3 animate-spin" /> : <ArrowUpCircle className="w-3 h-3" />}
