...
export const DirtyWorktreeConfirmation: React.FC = () => {
  ...
  const copy = isPolish
    ? {
        ...
        snapshot:
          "Przed wymuszeniem zostanie utworzona nowa migawka bezpieczeństwa, aby można było wykonać rollback.",
    ...
    : {
        ...
        snapshot:
          "A new safety snapshot will be created before the forced update so rollback remains available.",
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="dirty-update-title"
    >
      <button
      ...
      <section className="relative w-full max-w-lg rounded-2xl border border-amber-500/35 bg-card p-6 shadow-2xl">
        ...
        <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <button
            onClick={confirmDirtyUpdate}
            className="rounded-lg bg-amber-500 px-4 py-2 text-xs font-semibold text-amber-950 transition-colors hover:bg-amber-400"
          >
            {copy.confirm}
          </button>
        </div>
