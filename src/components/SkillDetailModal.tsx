...
export const SkillDetailModal: React.FC = () => {
  ...
  const handleRollback = async () => {
    if (!confirm('Are you sure you want to rollback to the selected backup point?')) return;
        ...
        >
          <div>
            ...
            <div className="flex items-start justify-between gap-4 pb-4 border-b border-border">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {selectedSkill.updateAvailable && selectedSkill.latestVersion && selectedSkill.latestVersion !== selectedSkill.currentVersion && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-500/15 text-amber-400 border border-amber-500/30">
                      {t.updateAvailable}: {formatSkillVersion(selectedSkill.latestVersion, language)}
                    </span>
                  )}
                </div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Author: <span className="text-foreground font-medium">{selectedSkill.author}</span>
                </p>
              </div>
              ...
              <div className="mt-4 p-4 rounded-xl border border-primary/40 bg-primary/10 animate-pulse flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Tworzenie migawki bezpieczeństwa, aktualizacja manifestu i synchronizacja plików we wszystkich lokalizacjach ({selectedSkill.installedLocations?.length || 1}).
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="mt-4 p-2.5 rounded-lg bg-muted/40 border border-border flex items-center justify-between gap-2 text-xs font-mono">
              <span className="truncate text-muted-foreground select-all">{selectedSkill.path}</span>
              <button
                onClick={copyPath}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-5">
              ...
              <div className="p-4 rounded-xl border border-border bg-card/50">
                <div className="flex items-center justify-between text-xs font-semibold text-foreground mb-2">
                  ...
                  {selectedSkill.remoteUrl && (
                    ...
                    >
                      <RefreshCw className={`w-2.5 h-2.5 ${isCheckingGitHub ? 'animate-spin text-primary' : ''}`} />
                ...
                <div className="space-y-1.5 text-xs text-muted-foreground">
                  <div className="flex justify-between items-center">
                    ...
                    {selectedSkill.remoteUrl ? (
                      ...
                      >
                        <span className="truncate">{selectedSkill.remoteUrl.replace('https://', '')}</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-foreground font-mono">{selectedSkill.branchOrTag || 'HEAD'}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {selectedSkill.permissions.length > 0 ? (
                    selectedSkill.permissions.map((p) => (
                      <span key={p} className="px-1.5 py-0.5 rounded text-[10px] bg-amber-500/10 text-amber-400 border border-amber-500/20 font-mono">
                        {p}
                      </span>
                    ))
                  ) : (
                    <span className="text-xs text-muted-foreground">Standard permissions</span>
                  )}
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl border border-border bg-card/50 sm:col-span-2">
                <div className="flex items-center gap-2 text-xs font-semibold text-foreground mb-2">
                  ...
                  <span>{t.compatibility} &amp; {t.targetEnvironment}</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-muted-foreground">
                  <div className="p-2.5 rounded-lg bg-background/50 border border-border/60">
                    <p className="font-semibold text-foreground text-[11px] mb-1">{t.targetEnvironment}:</p>
                    <p className="text-foreground/90 font-medium">
                      {selectedSkill.compatibility || `${selectedSkill.agentScope.toUpperCase()} Environment`}
                    </p>
                  </div>
                  <div className="p-2.5 rounded-lg bg-background/50 border border-border/60">
                    <p className="font-semibold text-foreground text-[11px] mb-1">{t.updateCompatibility}:</p>
                    <p className={selectedSkill.updateAvailable ? 'text-amber-400 font-medium' : 'text-emerald-400 font-medium'}>
                      {selectedSkill.updateCompatibility || (selectedSkill.updateAvailable ? 'SemVer Minor: Wstecznie kompatybilna aktualizacja (brak zmian łamiących API)' : 'Wersja bieżąca jest stabilna i w pełni kompatybilna')}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {selectedSkill.installedLocations && selectedSkill.installedLocations.length > 0 && (
                <div className="p-4 rounded-xl border border-border bg-card/50 sm:col-span-2">
                  <div className="flex items-center justify-between text-xs font-semibold text-foreground mb-2">
                    <div className="flex items-center gap-2">
                      <Folder className="w-4 h-4 text-primary" />
                      <span>{t.installedLocations} ({selectedSkill.installedLocations.length})</span>
                    </div>
                  ...
                  <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                    {selectedSkill.installedLocations.map((loc, idx) => (
                      ...
                      >
                        <span className="text-foreground/90 truncate mr-2 text-[11px]" title={loc}>
                          {loc}
                        </span>
            ...
            {selectedSkill.isGitRepo && (
              <div className="mt-5 p-4 rounded-xl border border-border bg-card/50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {isInstallingCustom ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <GitBranch className="w-3.5 h-3.5" />}
                    {t.installTag}
                  </button>
                </div>
              </div>
